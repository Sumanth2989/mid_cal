...
class AutoSaveObserver:
    def notify(self, calculation: Calculation, history: "History") -> None:
        # Use the history's current cfg, not a module-level snapshot
...
class History:
    def __init__(self, max_size: int, config: Optional[object] = None):
        ...
        config: pass calculator_config.cfg so paths follow the current env
        """
    ...
    def save_to_csv(self, path: Optional[str] = None):
        try:
            ...
            # Only create directories when using the DEFAULT history file.
            # If a custom path is passed, don't create dirs; let it fail for tests.
            if path is None:
    ...
    def load_from_csv(self, path: Optional[str] = None):
        from .calculation import Calculation  # avoid import cycle
